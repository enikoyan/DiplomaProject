...
var getServerKeyAPI = "https://localhost:44370/api/auth/getRecoveryKey?userEmail=";

window.addEventListener('DOMContentLoaded', async () => {
    if (localStorage.getItem('serverKey') != null) {
        location.href = 'https://localhost:44354/auth/recoveryPassword';
    }
        ...
        fetch(getServerKeyAPI += emailInput.value)
            ...
            .then(async response => {
                if (response) {
                    ...
                    await createMessagePopup("success", "Письмо для восстановления пароля успешно отправлено на Вашу почту!");
    ...
    async function goToSecondPart() {
        ...
        if (localStorage.getItem('recoveryTimer_started') === null || localStorage.getItem('recoveryTimer_ended') == "true") {
